...
const Leaderboard: React.FC = () => {
  ...
  return (
    <div className="max-w-6xl mx-auto">
      <div className="mb-8">
        <h1 className="text-3xl font-bold text-secondary-900">Leaderboard</h1>
        <p className="text-secondary-600 mt-2">See who's leading the collaboration game</p>
      </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
          ...
          {leaderboard[1] && (
            <div className="order-2 md:order-1">
              <div 
                className="block bg-white rounded-lg shadow-sm border border-secondary-200 p-6 text-center hover:shadow-md transition-shadow"
              >
                ...
                <div className="flex justify-center mb-4">
                  <UserHoverCard userId={leaderboard[1]._id}>
                    <img
                      ...
                      className="h-16 w-16 rounded-full border-4 border-gray-300 cursor-pointer"
                ...
                <UserHoverCard userId={leaderboard[1]._id}>
                  <h3 
                    className="text-lg font-semibold text-secondary-900 mb-2 hover:text-primary-600 transition-colors cursor-pointer"
                    onClick={() => window.location.href = `/app/profile/${leaderboard[1]._id}`}
                  >
                    {leaderboard[1].name}
                  </h3>
                </UserHoverCard>
                <p className="text-sm text-secondary-600 mb-2">Level {leaderboard[1].level}</p>
          ...
          <div className="order-1 md:order-2">
            ...
            >
              ...
              <UserHoverCard userId={leaderboard[0]._id}>
                <h3 
                  className="text-xl font-bold text-secondary-900 mb-2 hover:text-primary-600 transition-colors cursor-pointer"
                  onClick={() => window.location.href = `/app/profile/${leaderboard[0]._id}`}
                >
                  {leaderboard[0].name}
                </h3>
              </UserHoverCard>
              <p className="text-sm text-secondary-600 mb-2">Level {leaderboard[0].level}</p>
              <div className="space-y-1">
              ...
              <div className="flex items-center justify-between">
                <div className="flex items-center space-x-4">
                  ...
                  <div>
                    <UserHoverCard userId={user._id}>
                      <span
                        className="text-lg font-medium text-secondary-900 hover:text-primary-600 transition-colors cursor-pointer"
                        onClick={() => window.location.href = `/app/profile/${user._id}`}
                      >
                        {user.name}
                      </span>
                    </UserHoverCard>
                    <p className="text-sm text-secondary-600">Level {user.level}</p>
                  </div>
                </div>

                <div className="flex items-center space-x-6">
                  <div className="text-center">
                    <p className="text-sm text-secondary-500">CollabPoints</p>
      ...
      {leaderboard.length === 0 && (
        <div className="text-center py-12">
          <TrophyIcon className="h-16 w-16 text-secondary-400 mx-auto mb-4" />
